...
class True_node : public MEMORY_OBJECT
{
	public:
		...
		boost::array<boost::array<double, noa>, nob> get_exp_payoffs(int level) const
		{
			return m_exp_payoffs[level];
		}

		boost::array<boost::array<double,nob>, noT> get_belief_parameters() const
		{
			return m_belief_parameters;
		}

		/*boost::array<double,nob> get_belief_probabilities(int level) const
		{
			return m_belief_probabilities[level];
		}	*/

		boost::array<boost::array<double,noi>, noT> get_irr_beliefs() const
		{
			return m_irr_belief;
		}

		/*boost::array<double, noi> get_irr_probabilities(int level) const
		{
		...
		boost::array<boost::array<double, noi>, noT+1> get_shifts() const
		{
			return m_shifts;
		}
...
1.0/temperature*(m_payoff[action] + exploration_constant*sqrt(static_cast<double>(current_count)/(m_action_count[action]))):max_val);
	...
	private:
		...
		//boost::array<int, noa> m_action_count;
		boost::array<boost::array<boost::array<double,noa>,nob>, noT+1>  m_exp_payoffs;
		boost::array<boost::array<bool, nob>, noT+1> m_sufficient_exploration;
		boost::array<boost::array<double,nob>, noT> m_belief_parameters;
		boost::array<boost::array<double,noi>, noT> m_irr_belief;
		//boost::array<boost::array<double,noi>, noT> m_irr_probabilities;
		boost::array<boost::array<double, noi>, noT+1> m_shifts;
		//boost::array<boost::array<double,nob>, noT> m_belief_probabilities;
		//int m_free_child;
		//int m_free_counter;
		boost::array<bool, noT+1> m_expectation_exists;
		//boost::array<boost::array<bool, nob>, noT+1> m_rolled_out;
		boost::array<double , noT+1> m_expectation;
};
