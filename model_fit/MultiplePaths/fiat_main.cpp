...
// Multiround Trustgame Estimation by A.Hula 2016, last changes, exact calculation
...
void belief_updates(True_node** start_node, True_node** end_node, int const& level, double const& monetary_action, int const& act)
{
	True_node* origin_node;
	origin_node = *start_node;
	True_node* target_node;
	...
	boost::array<boost::array<double, noa>, nob> i_exp_payoffs = origin_node ->get_exp_payoffs(0);
			...
			{		
				for(int first_ret = 0; first_ret < nor(money_invested); ++first_ret)
				{
					...
					if(!(money_invested==0))
					{
						if(k2>0)
						{
							belief_updates(&Trustee_node_r, &current_node, k2, trustee_monetary_response , response[game]);
						}
					...
					inv_exp_payoffs = current_node -> get_exp_payoffs(0);
					local_shifts = current_node -> get_shifts();
					for(int irr=0; irr < noi; ++irr)
					{
			...
			{		
				...
				{	
					...
					{	
						...
						belief_updates(&current_node, &Trustee_holder, k2+1, investor_monetary_action , action[game+1]);	
		...
		if((planning_horizon- game) == 2)
		{
			...
			{			
				for(int first_act = 0; first_act < noa; ++first_act)
				{
					...
					if(k1>0)
					{
						belief_updates(&current_node, &Trustee_node_r, k1, investor_monetary_action, action[game]);
					}
					...
					for(int first_ret = 0; first_ret < nor(first_act); ++first_ret) 
					{
						...
						if(!action[game]==0)
						{
							belief_updates(&Trustee_node_r, &investor_holder,  k1+1, trustee_monetary_action, response[game]);
...
{			
	...
	static int d1;
	static int d2;
	static int no_sub = 400; //808
	static int start_id;
	static int end_id;
	int post =0;

	ifstream ifile ("/data/ahula/LargeSE.bin", ios::in | ios::binary| ios::ate);
	...
	static boost::array<boost::array<int,2>,8100> Subject_Irritability;	
	...
	/*for(int sub_id=0; sub_id < no_sub; ++sub_id) 
	{
		...
		il += sizeof *(int*)&melblock[il];	
		Subject_Bel_Repair[sub_id][0] = *(int*)&melblock[il];	
		il += sizeof *(int*)&melblock[il];
		Subject_Bel_Irritability[sub_id][0] = *(int*)&melblock[il];	
		il += sizeof *(int*)&melblock[il];		
		Subject_Sim_Repair[sub_id][0] = *(int*)&melblock[il];	
		il += sizeof *(int*)&melblock[il];
		Subject_Sim_Irritability[sub_id][0] = *(int*)&melblock[il];	
		il += sizeof *(int*)&melblock[il];		
		Subject_As_Repair[sub_id][0] = *(int*)&melblock[il];	
		il += sizeof *(int*)&melblock[il];
		Subject_As_Irritability[sub_id][0] = *(int*)&melblock[il];	
		il += sizeof *(int*)&melblock[il];			
		Subject_Inf_Repair[sub_id][0] = *(int*)&melblock[il];	
		il += sizeof *(int*)&melblock[il];
		Subject_Inf_Irritability[sub_id][0] = *(int*)&melblock[il];	
		il += sizeof *(int*)&melblock[il];			
		...
		il += sizeof *(int*)&melblock[il];	
		Subject_Bel_Repair[sub_id][1] = *(int*)&melblock[il];	
		il += sizeof *(int*)&melblock[il];
		Subject_Bel_Irritability[sub_id][1] = *(int*)&melblock[il];	
		il += sizeof *(int*)&melblock[il];		
		Subject_Sim_Repair[sub_id][1] = *(int*)&melblock[il];	
		il += sizeof *(int*)&melblock[il];
		Subject_Sim_Irritability[sub_id][1] = *(int*)&melblock[il];	
		il += sizeof *(int*)&melblock[il];	
		Subject_As_Repair[sub_id][1] = *(int*)&melblock[il];	
		il += sizeof *(int*)&melblock[il];
		Subject_As_Irritability[sub_id][1] = *(int*)&melblock[il];	
		il += sizeof *(int*)&melblock[il];			
		Subject_Inf_Repair[sub_id][1] = *(int*)&melblock[il];	
		il += sizeof *(int*)&melblock[il];
		Subject_Inf_Irritability[sub_id][1] = *(int*)&melblock[il];	
		il += sizeof *(int*)&melblock[il];		
	}
	...
	int investor_choice;
	int trustee_choice;
	int investor_irr_belief= 0;
	int trustee_irr_belief = 0;
	...
	ofstream ofs("/data/ahula/Param/NiceFiatBreakShift.bin", ofstream::out| ofstream::binary);			
	int iteration = 1;
	...
	for(int sub_id= start_id; sub_id < end_id; ++sub_id)
	{
		...
		Subject_Games[sub_id][0][1]= 4;

		Subject_Games[sub_id][1][0]= 2;
		...
		*/
		...
		Subject_Games[sub_id][1][0]= 2;
		Subject_Games[sub_id][1][1]= 3;		
		
		Subject_ToM[sub_id][0]=2;
		Subject_ToM[sub_id][1]=1;
		Subject_Guilt[sub_id][0]=2;
		Subject_Guilt[sub_id][1]=1;
		inv_risk_aversion = risk_grid[3];
		tru_risk_aversion = risk_grid[3];		
		investor_temperature =  temperature_grid[1];
		trustee_temperature = temperature_grid[1];	
		d1 =Planning[3]; 
		d2 =Planning[3];	
		investor_irr =2; 
		trustee_irr =0;
		investor_irr_belief = 0;
		trustee_irr_belief = 0;
					...
					//{
						...
						for(int time_step=0; time_step < global_time_horizon; ++time_step)	//global_time_horizon
						{
							...
							for(int first_action = 0; first_action < noa; ++first_action)
							{
								...
								if((Subject_ToM[sub_id][0]+3)%2== 0)
								{
									belief_updates(&investor_node, &Trust, 1, investor_monetary_action , first_action);
								...
								if((Subject_ToM[sub_id][1]+3)%2== 1 )
								{
									belief_updates(&trustee_node, &Trust, 1, investor_monetary_action, first_action);
							...
							for(int level = 0; level <= Subject_ToM[sub_id][0]; ++level)
							{
								if((Subject_ToM[sub_id][0]+level+2)%2== 0)
								{
									...
									main_i_exp_payoffs = investor_node->get_exp_payoffs(0);
						...
						//Run Trustee Simulations
							...
							if(time_step > 1)
							{
								investor_choice = softmax( hold_investor_payoffs[Subject_ToM[sub_id][0]][time_step][investor_irr][Subject_Guilt[sub_id][0]], 201202);
								Subject_Games[sub_id][0][time_step] = investor_choice;
							}else{
								investor_choice= Subject_Games[sub_id][0][time_step];
							}
							...
							for(int level =1; level <= Subject_ToM[sub_id][1]+1 ; ++level) //all initial investor considerations
							{
								...
								{ 
									...
									belief_updates(&trustee_node, &temp_node, level, investor_monetary_action, Subject_Games[sub_id][0][time_step]);
							...
							if(time_step > 1){
								...
								Subject_Games[sub_id][1][time_step] = trustee_choice;
							}
							else{
								trustee_choice = Subject_Games[sub_id][1][time_step];
							}
							...
							{	
								...
								{ 
									...
									belief_updates(&investor_node, &temp_node, level, investor_monetary_action, Subject_Games[sub_id][0][time_step]);								
											...
											*/
											for(int time_step =0; time_step < global_time_horizon; ++time_step)
											{
												...
												for(int k =0; k < noT; ++k)
												{
													...
													for(int irr_belief=0; irr_belief < noi; ++irr_belief)
													{													
														WI_irr_beliefs[time_step][k][irr_belief]=store_investor_irr_beliefs[time_step][k][irr_belief];
														WI_investor_shifts[time_step][k][irr_belief] = store_investor_shifts[time_step][k][irr_belief];
														WI_investor_shifts[time_step][k+1][irr_belief] = store_investor_shifts[time_step][k+1][irr_belief];
													}
													for(int guilt_belief=0; guilt_belief < nob; ++guilt_belief)
													{
														for(int irr_belief=0; irr_belief < noi; ++irr_belief)
														{			
															for(int p_act =0; p_act  < noa; ++p_act )
															{
																...
																WI_investor_payoffs[k][time_step][irr_belief][guilt_belief][p_act] = hold_investor_payoffs[k][time_step][irr_belief][guilt_belief][p_act];
															}
														}
													}														
												}
											...
											*/
											for(int time_step =0; time_step < global_time_horizon; ++time_step)
											{
												...
												for(int k =0; k < noT; ++k)
												{
													...
													for(int irr_belief=0; irr_belief < noi; ++irr_belief)
													{													
														WT_irr_beliefs[time_step][k][irr_belief]=store_trustee_irr_beliefs[time_step][k][irr_belief];
														WT_trustee_shifts[time_step][k][irr_belief] = store_trustee_shifts[time_step][k][irr_belief];
														WT_trustee_shifts[time_step][k+1][irr_belief] = store_trustee_shifts[time_step][k+1][irr_belief];
													}	
													for(int guilt_belief=0; guilt_belief < nob; ++guilt_belief)
													{
														for(int irr_belief=0; irr_belief < noi; ++irr_belief)
														{			
															...
															{
																//cout << "low level shift" << trustee_shifts[0][1] << endl;
																WT_trustee_payoffs[k][time_step][irr_belief][guilt_belief][p_act] = hold_trustee_payoffs[k][time_step][irr_belief][guilt_belief][p_act];
															}
														}
													}														
												}
