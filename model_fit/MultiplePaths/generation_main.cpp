...
void belief_updates(True_node** start_node, True_node** end_node, int const& level, double const& monetary_action, int const& act, bool const& investor, bool const& trustee)
{
	True_node* origin_node;
	origin_node = *start_node;
	True_node* target_node;
...
void investor_expectation_calculation( int const& level, True_node** given_pointer, boost::array<double, nob> const& running_belief_probability , boost::array<double, noi> const& irr_probability, bool const& trustee)
{	
	double expectation = 0.0; 
				...
				for(int first_ret = 0; first_ret < nor(money_invested); ++first_ret)
				{
					...
					if(!current_node)
					{
						...
						if((k2+2)%2== 0)
						{
							...
							investor_expectation_calculation( 0, &current_node,  running_belief_probability , running_irr_probability, trustee_mode);
						}
						else
						{
							for(int guilt_counter=0; guilt_counter < nob; ++guilt_counter)
							...
							running_irr_probability[0] = 1.0;					
							investor_expectation_calculation( -1, &current_node,  running_belief_probability , running_irr_probability, trustee_mode);
						}											
					}
					...
					if(!(money_invested==0))
					{
						if(k2>0)
						{
							belief_updates(&Trustee_node_r, &current_node, k2, trustee_monetary_response , response[game], false, trustee_mode);
						}
					...
					{	
						if(k2>0)
						{
							...
							investor_expectation_calculation( k2, &current_node,  running_belief_probability , running_irr_probability, trustee_mode);
						}
						else
						{
							for(int guilt_counter=0; guilt_counter < nob; ++guilt_counter)
			...
			{		
				...
				{	
					...
					{	
						...
						belief_updates(&current_node, &Trustee_holder, k2+1, investor_monetary_action , action[game+1], true, trustee_mode);	
	...
	else
	{
		...
		if((planning_horizon- game) == 1)
		{
			...
			if(k1>0) 
			{
				...
				investor_expectation_calculation( k1-1, &current_node,  running_belief_probability , running_irr_probability, trustee_mode);					
			}	
		...
		if((planning_horizon- game) == 2)
		{
			...
			{			
				for(int first_act = 0; first_act < noa; ++first_act)
				{
					...
					if(k1>0)
					{
						belief_updates(&current_node, &Trustee_node_r, k1, investor_monetary_action, action[game], true, trustee_mode);
					}
					...
					for(int first_ret = 0; first_ret < nor(first_act); ++first_ret) 
					{
						...
						if(!investor_holder)
						{
							...
							if((k1+2)%2== 1)
							{
								...
								investor_expectation_calculation( 0, &investor_holder,  running_belief_probability , running_irr_probability, trustee_mode);
							}
							else
							{
								for(int guilt_counter=0; guilt_counter < nob; ++guilt_counter)
								...
								running_irr_probability[0] = 1.0;					
								investor_expectation_calculation( -1, &investor_holder, running_belief_probability , running_irr_probability, trustee_mode);
							}															
						}
						for(int b=0; b < nob; ++b)
						{
						...
						if(!action[game]==0)
						{
							belief_updates(&Trustee_node_r, &investor_holder,  k1+1, trustee_monetary_action, response[game], false, trustee_mode);
						...
						if(k1 > 0)
						{
							...
							investor_expectation_calculation( k1-1, &investor_holder,  running_belief_probability , running_irr_probability, trustee_mode);
						}
						else
						{
							for(int guilt_counter=0; guilt_counter < nob; ++guilt_counter)
...
{			
	...
	static int d1;
	static int d2;
	static int no_sub = 93;//338; //808
	static int start_id;
	static int end_id;
	int post =0;

	ifstream ifile ("/data/ahula/Generative/BrooksFlexiblePar.bin", ios::in | ios::binary| ios::ate);
	...
	static boost::array<boost::array<int,2>,1000> Subject_Guilt;
	static boost::array<boost::array<int,2>,1000> Subject_ToM;
	static boost::array<boost::array<int,2>,1000> Subject_Plan;
	static boost::array<boost::array<int,2>,1000> Subject_Temp;
	static boost::array<boost::array<int,2>,1000> Subject_Risk;
	static boost::array<boost::array<int,2>,1000> Subject_Shift;
	double trustee_monetary_response;
	double investor_monetary_action;
	//static boost::array<boost::array<int,2>,8100> Subject_Repair;
	static boost::array<boost::array<int,2>,1000> Subject_Irritability;/*
	static boost::array<boost::array<int,2>,8100> Subject_Bel_Repair;
	static boost::array<boost::array<int,2>,8100> Subject_Bel_Irritability;
	static boost::array<boost::array<int,2>,8100> Subject_Sim_Repair;
	static boost::array<boost::array<int,2>,8100> Subject_Sim_Irritability;
	static boost::array<boost::array<int,2>,8100> Subject_As_Repair;
	static boost::array<boost::array<int,2>,8100> Subject_As_Irritability;
	static boost::array<boost::array<int,2>,8100> Subject_Inf_Repair;
	static boost::array<boost::array<int,2>,8100> Subject_Inf_Irritability;	*/
	for(int sub_id=0; sub_id < no_sub; ++sub_id)
	{
		Subject_Guilt[sub_id][0] = *(int*)&melblock[il];
		il += sizeof *(int*)&melblock[il];
		Subject_Plan[sub_id][0] = *(int*)&melblock[il];
		il += sizeof *(int*)&melblock[il];
		Subject_ToM[sub_id][0] = *(int*)&melblock[il];
		il += sizeof *(int*)&melblock[il];
		Subject_Temp[sub_id][0] = *(int*)&melblock[il];
		il += sizeof *(int*)&melblock[il];
		Subject_Risk[sub_id][0] = *(int*)&melblock[il];
		il += sizeof *(int*)&melblock[il];
		Subject_Shift[sub_id][0] = *(int*)&melblock[il];
		il += sizeof *(int*)&melblock[il];
		//Subject_Repair[sub_id][0] = *(int*)&melblock[il];
		//il += sizeof *(int*)&melblock[il];
		Subject_Irritability[sub_id][0] = *(int*)&melblock[il];
		il += sizeof *(int*)&melblock[il];
		Subject_Guilt[sub_id][1] = *(int*)&melblock[il];
		il += sizeof *(int*)&melblock[il];
		Subject_Plan[sub_id][1] = *(int*)&melblock[il];
		il += sizeof *(int*)&melblock[il];
		Subject_ToM[sub_id][1] = *(int*)&melblock[il];
		il += sizeof *(int*)&melblock[il];
		Subject_Temp[sub_id][1] = *(int*)&melblock[il];
		il += sizeof *(int*)&melblock[il];
		Subject_Risk[sub_id][1] = *(int*)&melblock[il];
		il += sizeof *(int*)&melblock[il];
		Subject_Shift[sub_id][1] = *(int*)&melblock[il];
		il += sizeof *(int*)&melblock[il];
		//Subject_Repair[sub_id][1] = *(int*)&melblock[il];
		//il += sizeof *(int*)&melblock[il];
	...
	ofstream ofs("/data/ahula/Generative/FlexibleAvCurve.bin", ofstream::out| ofstream::binary);			
	int iteration = 1;
	...
	for(int sub_id= start_id; sub_id < end_id; ++sub_id)
	{
		...
		Ilike_max = 1000;
		cout << sub_id <<endl;
		Subject_Games[sub_id][0][0]= 2;
		Subject_Games[sub_id][0][1]= 1;
		Subject_Games[sub_id][0][2]= 1;
		Subject_Games[sub_id][0][3]= 0;
		Subject_Games[sub_id][0][4]= 1;
		Subject_Games[sub_id][0][5]= 1;
		Subject_Games[sub_id][0][6]= 0;
		Subject_Games[sub_id][0][7]= 1;
		Subject_Games[sub_id][0][8]= 1;
		Subject_Games[sub_id][0][9]= 1;
		Subject_Games[sub_id][1][0]= 3;
		Subject_Games[sub_id][1][1]= 1;
		Subject_Games[sub_id][1][2]= 0;
		Subject_Games[sub_id][1][3]= 0;
		Subject_Games[sub_id][1][4]= 1;
		Subject_Games[sub_id][1][5]= 0;
		Subject_Games[sub_id][1][6]= 0;
		Subject_Games[sub_id][1][7]= 4;
		Subject_Games[sub_id][1][8]= 2;
		Subject_Games[sub_id][1][9]= 0;
		Subject_ToM[sub_id][0]=4;
		Subject_ToM[sub_id][1]=3;
		Subject_Guilt[sub_id][0]=1;
		Subject_Guilt[sub_id][1]=2;
		 Subject_Risk[sub_id][0] = 5;
		 Subject_Risk[sub_id][1]= 1;		
		Subject_Temp[sub_id][0] =  3;
		Subject_Temp[sub_id][1] = 0;	
		Subject_Plan[sub_id][0] =2; 
		Subject_Plan[sub_id][1] =3;	
		Subject_Irritability[sub_id][0] =0; 
		Subject_Irritability[sub_id][1] =0;
		Subject_Shift[sub_id][0] = 0;
		Subject_Shift[sub_id][1] = 0;		
			//for(int irritability_count=0; irritability_count < 6; ++irritability_count)
			//{
	
		int investor_risk_count = Subject_Risk[sub_id][0];
		int trustee_risk_count = Subject_Risk[sub_id][1];
			inv_risk_aversion = risk_grid[investor_risk_count];
			tru_risk_aversion = risk_grid[trustee_risk_count];

		//int temp_count = 3; 
		int investor_temp_count = Subject_Temp[sub_id][0];
		int trustee_temp_count = Subject_Temp[sub_id][1];
				...
				if(tru_risk_aversion < 1.2)
				{
					tru_risk_aversion = 1.0;
					trustee_utpi_init = initialize_utpi_init(1.0);
					trustee_investor_choice_likelihood = initialize_choice_likelihood(investor_utpi_init, investor_temperature);

					for(int system=0; system < nob; ++system)
					{
						trustee_ui_init_system[system] = initialize_ui_init(system, inv_risk_aversion);
						//trustee_opt_system[system] = initialize_ui_init(system, 1.0);
						trustee_ui = initialize_investor_utility(investor_ui_init_system[system], investor_investor_choice_likelihood);
						trustee_max_ui_index = initialize_max_ui_index(investor_ui);
					}

					trustee_offer_utility=initialize_offer_utility(investor_investor_choice_likelihood, inv_risk_aversion);
					trustee_trustee_choice_likelihood = initialize_trustee_choice_likelihood(investor_offer_utility, investor_temperature);
					for(int system=0; system < nob; ++system)
					{
						trustee_ut_init_system[system] = initialize_trustee_utility(system, 1.0);
						trustee_max_ut_index=initialize_max_ut_index(investor_ut_init_system[system]);
						trustee_trustee_probabilities[system]=initialize_trustee_probabilities(investor_ut_init_system[system], investor_temperature);
					}
					trustee_expected_trustee_payoff  = initialize_expected_trustee_payoff(investor_trustee_probabilities, investor_ut_init_system);
					trustee_expected_outcome_utility = initialize_expected_outcome_utility(investor_trustee_probabilities, investor_ui_init_system);
					for(int guilt_counter = 0; guilt_counter < nob ; ++guilt_counter)
					{
						guilt_parameter[noT]=guilt_counter;
						investor_k( 0
				...
				if(inv_risk_aversion > 1.0)
				{
					...
					int investor_plan = Subject_Plan[sub_id][0];
					int trustee_plan = Subject_Plan[sub_id][1];		
					d1 =Planning[investor_plan]; 
					d2 =Planning[trustee_plan];
				//int irr_belief = 0;
					int investor_irr_belief = Subject_Shift[sub_id][0];
					int trustee_irr_belief = Subject_Shift[sub_id][1];
						...
						{						
							for(int irr =0; irr < noi; ++irr)
							{
								investor_irr_beliefs[level][irr] = irr_belief_presets[investor_irr_belief][irr];
								trustee_irr_beliefs[level][irr] = irr_belief_presets[trustee_irr_belief][irr];				
							}
						}
						++countr;
						...
						investor_irr=Subject_Irritability[sub_id][0];
						trustee_irr=Subject_Irritability[sub_id][1];									
						for(int time_step=0; time_step < global_time_horizon; ++time_step)	//global_time_horizon
						{
							...
							{			
								...
								{ 		
									...
									investor_expectation_calculation( 0, &trustee_node, main_running_belief_probability , main_irr_probability, true);
								...
								else
								{
									...
									investor_expectation_calculation( -1, &trustee_node,  main_running_belief_probability , main_irr_probability, true);				
								}	
							}				
							...
							{			
								...
								{ 		
									...
									investor_expectation_calculation( 0, &investor_node, main_running_belief_probability , main_irr_probability, false);			
								}		
								else
								{
									...
									main_irr_probability[0]= 1.0;
									investor_expectation_calculation( -1, &investor_node,  main_running_belief_probability , main_irr_probability, false);						
								}							
							}	
							...
							for(int first_action = 0; first_action < noa; ++first_action)
							{
								...
								if((Subject_ToM[sub_id][0]+3)%2== 0)
								{
									belief_updates(&investor_node, &Trust, 1, investor_monetary_action , first_action, true, false);
								...
								if((Subject_ToM[sub_id][1]+3)%2== 1 )
								{
									belief_updates(&trustee_node, &Trust, 1, investor_monetary_action, first_action, true, true);
								...
								{ 		
									...
									if(level <(Subject_ToM[sub_id][0]+1))
									{
										...
										{	
											...
											investor_expectation_calculation( level, &investor_node,  main_running_belief_probability , main_irr_probability, false);
										}
						...
						//Run Trustee Simulations
								...
								{ 				
									...
									{					
										...
										{	
											...
											investor_expectation_calculation( level, &trustee_node,  main_running_belief_probability , main_irr_probability, true);
										}	
									}
							...
							for(int reprod_counter = 0; reprod_counter < 200; ++reprod_counter)
							{
								int investor_choice = softmax( hold_investor_payoffs[Subject_ToM[sub_id][0]][time_step][investor_irr][Subject_Guilt[sub_id][0]], 201202);
								ofs.write( reinterpret_cast<char*>( &investor_choice ), sizeof investor_choice);	
							}
							...
							for(int level =1; level <= Subject_ToM[sub_id][1]+1 ; ++level) //all initial investor considerations
							{
								...
								{ 
									...
									belief_updates(&trustee_node, &temp_node, level, investor_monetary_action, Subject_Games[sub_id][0][time_step], true, true);
							...
							{							
								if(!(Subject_Games[sub_id][0][time_step]==0))
								{
									trustee_choice = softmax( hold_trustee_payoffs[Subject_ToM[sub_id][1]][time_step][trustee_irr][Subject_Guilt[sub_id][1]], 201202);
								}
								else
								{
									trustee_choice = 0;
								}
								ofs.write( reinterpret_cast<char*>( &trustee_choice ), sizeof trustee_choice );
							}
							...
							{	
								...
								{ 
									...
									belief_updates(&investor_node, &temp_node, level, investor_monetary_action, Subject_Games[sub_id][0][time_step], true, false);								
